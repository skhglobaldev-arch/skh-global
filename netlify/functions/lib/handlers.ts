...
import { bookSlotTransaction, SlotUnavailableError } from './booking';
import {
  sendCallBookedEmail,
...
export const handleAdminRequest = async (request: Request) => {
  ...
  if (resource === 'stats' && method === 'GET') {
    const [contacts, reviews, bookings, slots] = await Promise.all([
      ...
      db.collection(COLLECTIONS.availabilitySlots).where('status', '==', 'Available').get(),
    ]);
  ...
  if (resource === 'slots') {
    if (method === 'GET' && !id) {
      const snapshot = await db.collection(COLLECTIONS.availabilitySlots).orderBy('date').orderBy('time').limit(200).get();
      return json({ items: snapshot.docs.map((doc) => serializeDoc(doc.id, doc.data() as Record<string, unknown>)) });
    }
    if (method === 'POST' && !id) {
      const body = (await request.json()) as AvailabilitySlot;
      if (!body.date || !body.time || !body.timezone) {
        return json({ error: 'date, time, and timezone are required.' }, 400);
      }
      const docRef = await db.collection(COLLECTIONS.availabilitySlots).add({
        date: body.date,
        time: body.time,
        timezone: body.timezone,
        durationMinutes: body.durationMinutes || 30,
        status: (body.status || 'Available') as SlotStatus,
        createdAt: FieldValue.serverTimestamp(),
        updatedAt: FieldValue.serverTimestamp(),
      });
