...
import { Globe, ArrowRight, CheckCircle, Monitor, Smartphone, Zap, Shield, Sparkles, Settings } from 'lucide-react';
...
const getUnsplashUrl = (term: string) => `https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&q=80&w=1200&h=800&q=${encodeURIComponent(term)}`;

export const AppPreview: React.FC<AppPreviewProps> = ({ data }) => {
  if (!data) return null;

  const fontClass = data.fontStyle === 'serif' ? 'font-serif' : data.fontStyle === 'display' ? 'font-display' : 'font-sans';
  const isDark = data.layoutMode === 'dark';
    ...
    <div className={`w-full max-w-6xl mx-auto rounded-[3.5rem] overflow-hidden shadow-[0_60px_120px_rgba(0,0,0,0.7)] border border-white/10 ${isDark ? 'bg-slate-950 text-white' : 'bg-white text-slate-900'} ${fontClass} animate-in fade-in zoom-in-95 duration-1000 group/browser`}>
      ...
      <div className={`${isDark ? 'bg-slate-900 border-white/5' : 'bg-slate-50 border-slate-200'} border-b px-10 py-5 flex items-center justify-between`}>
        ...
        <div className={`flex items-center gap-3 ${isDark ? 'bg-black/40 text-slate-400' : 'bg-white text-slate-400 shadow-sm'} border border-white/10 px-10 py-2.5 rounded-full text-[11px] font-mono w-1/3 justify-center shadow-inner`}>
          <Globe size={12} className="opacity-50" /> {data.appName.toLowerCase().replace(/\s+/g, '-')}.skh.dev
        </div>
      ...
      <header className={`px-12 py-12 flex items-center justify-between sticky top-0 z-50 transition-all ${isDark ? 'bg-slate-950/80 backdrop-blur-3xl' : 'bg-white/80 backdrop-blur-3xl'}`}>
        <div className="flex items-center gap-4 group cursor-pointer">
           <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-2xl transition-transform group-hover:rotate-12" style={{ backgroundColor: data.primaryColor }}>
              {data.appName[0]}
           </div>
           <span className="font-black text-2xl tracking-tighter uppercase">{data.appName}</span>
        </div>
        <nav className="hidden lg:flex gap-14 text-[13px] font-black uppercase tracking-[0.25em] opacity-60">
          {data.navigation.map((item, i) => (
            <a key={i} href="#" className="hover:opacity-100 transition-all relative group">
              {item}
              <span className="absolute -bottom-2 left-0 w-0 h-1 rounded-full bg-current group-hover:w-full transition-all duration-300" style={{ backgroundColor: data.primaryColor }}></span>
            </a>
          ))}
        </nav>
        <button className="px-10 py-4 rounded-2xl text-[12px] font-black uppercase tracking-widest text-white shadow-2xl transition-all hover:scale-105 active:scale-95 flex items-center gap-3" style={{ backgroundColor: data.primaryColor }}>
          Get Initialized <ArrowRight size={18} />
        </button>
      </header>
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/30 to-transparent flex flex-col justify-center px-16 md:px-24">
          <div className="max-w-5xl space-y-12">
            <div className="inline-flex items-center gap-4 px-8 py-3 rounded-full border border-white/10 bg-white/5 backdrop-blur-3xl shadow-xl">
               <Sparkles size={18} style={{ color: data.primaryColor }} />
               <span className="text-[11px] font-black uppercase tracking-[0.4em] opacity-80">Secure Node Online</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-10 pt-8">
              <button className="px-14 py-7 rounded-[2rem] text-xl font-black text-white shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5)] transition-all hover:-translate-y-2 flex items-center justify-center gap-4" style={{ backgroundColor: data.primaryColor }}>
                Launch Interface <Zap size={24} />
              </button>
              ...
              <div key={idx} className="py-48 px-16 md:px-24">
                <div className="mb-28 flex flex-col md:flex-row md:items-end justify-between gap-10">
                  <div className="max-w-3xl">
                    ...
                    <h2 className="text-7xl font-black tracking-tighter leading-none">{section.title}</h2>
                  </div>
                  <div className="w-40 h-2.5 rounded-full" style={{ backgroundColor: data.primaryColor }}></div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-10 auto-rows-[350px]">
                  {section.items.map((item: any, i: number) => (
                    ...
                    >
                      <img src={`https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&q=80&w=800&h=600&sig=${i}`} className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover/card:scale-110 opacity-70" />
            ...
            return (
              <div key={idx} className="py-48 px-16">
                 <div className="text-center mb-32">
                   <h2 className="text-8xl font-black tracking-tighter mb-8">{section.title}</h2>
                   <p className="text-2xl opacity-40 font-light tracking-wide">Investment levels for sustained operational growth.</p>
                 </div>
                 <div className="grid md:grid-cols-2 gap-12 max-w-6xl mx-auto">
                    {section.plans.map((plan: any, i: number) => (
                      <div key={i} className={`p-20 rounded-[4.5rem] border-2 transition-all hover:scale-[1.03] ${plan.popular ? 'bg-white text-slate-950 border-white shadow-[0_60px_100px_rgba(255,255,255,0.05)]' : 'bg-slate-900/40 text-white border-white/10'}`}>
                        {plan.popular && <span className="inline-block px-8 py-2.5 bg-brand-500 text-white text-[11px] font-black uppercase tracking-[0.3em] rounded-full mb-12 shadow-2xl">Architect_Choice</span>}
                        <h3 className="text-4xl font-black mb-6 tracking-tight">{plan.name}</h3>
                        <div className="text-8xl font-black mb-16 tracking-tighter">{plan.price}<span className="text-xl font-medium opacity-30">/deployment</span></div>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center gap-24">
           <div className="space-y-8 text-center lg:text-left">
              <div className="flex items-center justify-center lg:justify-start gap-5">
                 ...
                 <span className="font-black text-4xl tracking-tighter uppercase">{data.appName}</span>
              </div>
              <p className="text-xl opacity-30 font-medium tracking-wide">© 2024 SYSTEM_PROTOCOL v5.0.1_STABLE / ENGINEERED BY SKH.GLOBAL</p>
           </div>
           <div className="flex flex-wrap justify-center gap-16 text-[12px] font-black uppercase tracking-[0.3em] opacity-40">
             <a href="#" className="hover:opacity-100 transition-opacity">Global_Hub</a>
             <a href="#" className="hover:opacity-100 transition-opacity">Security_Log</a>
             <a href="#" className="hover:opacity-100 transition-opacity">API_Reference</a>
             <a href="#" className="hover:opacity-100 transition-opacity">Contact_Arch</a>
           </div>
        </div>
      </footer>
    </div>
